...
/** Mod Menu integration for 1.19.2 (old ButtonWidget ctor, MatrixStack). */
public class QoLModMenuIntegration implements ModMenuApi {
    ...
    public static class QoLConfigScreen extends Screen {
        ...
        private void rebuildWidgets() {
            ...
            y += ENTRY_H; // space for "Client Features" label drawn in render()
            ...
            y += ENTRY_H; // space for "Hunger Display" label
            ...
            y += ENTRY_H; // space for "Compatibility" label
        ...
        public void render(MatrixStack matrices, int mx, int my, float delta) {
            ...
            DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer,
                    Text.literal("§6§lQoL Mod - Settings").asOrderedText(), cx, 12, 0xFFFFFF);
            DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer,
                    Text.literal("§e§lClient Features").asOrderedText(), cx, y + 6, 0xFFFFFF);
            int featY = y + ENTRY_H * 7 + 4;
            DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer,
                    Text.literal("§e§lHunger Display").asOrderedText(), cx, featY + 6, 0xFFFFFF);
            int compatY = featY + ENTRY_H * 3 + 4;
            DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer,
                    Text.literal("§e§lCompatibility").asOrderedText(), cx, compatY + 6, 0xFFFFFF);
    ...
    public static class HungerDisplaySettingsScreen extends Screen {
        ...
        protected void init() {
            ...
            satFieldY = y+14;
            satColorField = new TextFieldWidget(this.textRenderer, fieldX, satFieldY, fw, 18, Text.literal("Sat"));
            ...
            exhFieldY = y+14;
            exhaustColorField = new TextFieldWidget(this.textRenderer, fieldX, exhFieldY, fw, 18, Text.literal("Exh"));
            ...
            foodFieldY = y+14;
            foodColorField = new TextFieldWidget(this.textRenderer, fieldX, foodFieldY, fw, 18, Text.literal("Food"));
            ...
            int bw = 90;
            addDrawableChild(new ButtonWidget(cx - bw - 4, y, bw, 20,
                    Text.literal("Save & Back"), btn -> saveAndClose()));
            addDrawableChild(new ButtonWidget(cx + 4, y, bw, 20,
                    Text.literal("Cancel"), btn -> MinecraftClient.getInstance().setScreen(parent)));
        }
        ...
        public void render(MatrixStack matrices, int mx, int my, float delta) {
            ...
            int cx = this.width / 2;
            DrawableHelper.drawCenteredTextWithShadow(matrices, this.textRenderer,
                    Text.literal("§6§lHunger Display - Colours").asOrderedText(), cx, 16, 0xFFFFFF);
            DrawableHelper.drawTextWithShadow(matrices, this.textRenderer,
                    Text.literal("Saturation pip colour (hex RRGGBB)"), cx - 130, 48, 0xCCCCCC);
            DrawableHelper.drawTextWithShadow(matrices, this.textRenderer,
                    Text.literal("Exhaustion bar colour (hex RRGGBB)"), cx - 130, 86, 0xCCCCCC);
            DrawableHelper.drawTextWithShadow(matrices, this.textRenderer,
                    Text.literal("Food preview pip colour (hex RRGGBB)"), cx - 130, 124, 0xCCCCCC);
            super.render(matrices, mx, my, delta);
