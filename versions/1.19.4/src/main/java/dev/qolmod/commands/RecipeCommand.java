...
public class RecipeCommand {
    ...
    private static int executeRecipe(CommandContext<ServerCommandSource> ctx) {
        ...
        // Find recipes that produce this item — in 1.19.4, values() returns Recipe<?> directly
        var recipes = recipeManager.values().stream()
                .filter(entry -> {
                    ItemStack output = getRecipeResult(entry);
    ...
    /**
     * Gets the result of a recipe. In 1.19.4, Recipe has getOutput() with no args or DynamicRegistryManager.
     */
