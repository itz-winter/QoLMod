...
public class BanCommand {
    ...
    private static int executeBan(CommandContext<ServerCommandSource> ctx, String durationStr, String reason) {
        ...
        BannedPlayerEntry entry = new BannedPlayerEntry(new PlayerConfigEntry(target.getGameProfile()),
    ...
    private static int executeUnban(CommandContext<ServerCommandSource> ctx) {
        ...
        // Find the banned entry by name since getUserCache() was removed in 1.21.9
        PlayerConfigEntry foundEntry = null;
        for (var banEntry : banList.values()) {
            if (banEntry.getKey().name().equalsIgnoreCase(targetName)) {
                foundEntry = banEntry.getKey();
                break;
            }
        }

        if (foundEntry != null) {
            banList.remove(foundEntry);
