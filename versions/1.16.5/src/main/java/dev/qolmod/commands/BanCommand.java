...
public class BanCommand {
    ...
    private static int executeBan(CommandContext<ServerCommandSource> ctx, String durationStr, String reason) {
        ...
        if (target == null) {
            source.sendFeedback(new LiteralText("§cPlayer not found: " + targetName), false);
            return 0;
        }
        ...
        if (durationStr != null && !durationStr.equalsIgnoreCase("permanent")) {
            ...
            if (seconds <= 0) {
                source.sendFeedback(new LiteralText("§cInvalid duration format. Use: 1d2h30m"), false);
                return 0;
            }
        ...
        target.networkHandler.disconnect(new LiteralText("§cYou have been banned " + durationDisplay +
                ".\n§7Reason: " + reasonStr));

        source.sendFeedback(new LiteralText("§aBanned " + targetName + " " + durationDisplay + "."), false);
        if (reason != null) {
            source.sendFeedback(new LiteralText("§7Reason: " + reasonStr), false);
        }
    ...
    private static int executeUnban(CommandContext<ServerCommandSource> ctx) {
        ...
        BannedPlayerList banList = server.getPlayerManager().getUserBanList();
        com.mojang.authlib.GameProfile profile = server.getUserCache().findByName(targetName);

        if (profile != null) {
            if (banList.contains(profile)) {
                banList.remove(profile);
                source.sendFeedback(new LiteralText("§aUnbanned " + targetName + "."), false);
                return 1;
            }
        }

        source.sendFeedback(new LiteralText("§c" + targetName + " is not banned."), false);
        return 0;
    }
