...
public class QoLModMenuIntegration implements ModMenuApi {
    ...
    public static class QoLConfigScreen extends Screen {
        ...
        private void rebuildWidgets() {
            ...
            final int fy1 = y;
            addButton(new ButtonWidget(cx - BTN_W/2, fy1, BTN_W, BTN_H,
                    new LiteralText("Hunger Display Colours..."),
                    btn -> MinecraftClient.getInstance().openScreen(new HungerDisplaySettingsScreen(this))));
            y += ENTRY_H;
            ...
            final int fy2 = y;
            addButton(new ButtonWidget(cx - BTN_W/2, fy2, BTN_W, BTN_H,
                    new LiteralText("Done"),
                    btn -> { config.save(); MinecraftClient.getInstance().openScreen(parent); }));
        }
        ...
        public void render(MatrixStack matrices, int mx, int my, float delta) {
            ...
            int cx = this.width / 2, y = TOP_Y - scrollOffset;
            this.textRenderer.drawWithShadow(matrices, "QoL Mod - Settings", (float)(cx - this.textRenderer.getWidth("QoL Mod - Settings")/2), (float)(12), 0xFFAA00);
            this.textRenderer.drawWithShadow(matrices, "[ Client Features ]", (float)(cx - this.textRenderer.getWidth("[ Client Features ]")/2), (float)(y + 6), 0xFFEE44);
            int featY = y + ENTRY_H * 7 + 4;
            this.textRenderer.drawWithShadow(matrices, "[ Hunger Display ]", (float)(cx - this.textRenderer.getWidth("[ Hunger Display ]")/2), (float)(featY + 6), 0xFFEE44);
            int compatY = featY + ENTRY_H * 3 + 4;
            this.textRenderer.drawWithShadow(matrices, "[ Compatibility ]", (float)(cx - this.textRenderer.getWidth("[ Compatibility ]")/2), (float)(compatY + 6), 0xFFEE44);
            super.render(matrices, mx, my, delta);
        }

        @Override
        public void onClose() { config.save(); MinecraftClient.getInstance().openScreen(parent); }

        private void addToggle(int cx, int y, String label, String key) {
            boolean val = config.getBool(key);
            addButton(new ButtonWidget(cx - BTN_W/2, y, BTN_W, BTN_H,
    ...
    public static class HungerDisplaySettingsScreen extends Screen {
        ...
        protected void init() {
            ...
            this.children.add(satColorField); y += 38;
            ...
            this.children.add(exhaustColorField); y += 38;
            ...
            this.children.add(foodColorField); y += 48;

            int bw = 90;
            addButton(new ButtonWidget(cx-bw-4, y, bw, 20, new LiteralText("Save & Back"), btn -> saveAndClose()));
            addButton(new ButtonWidget(cx+4, y, bw, 20, new LiteralText("Cancel"),
                    btn -> MinecraftClient.getInstance().openScreen(parent)));
        }
        ...
        public void render(MatrixStack matrices, int mx, int my, float delta) {
            this.renderBackground(matrices);
            this.textRenderer.drawWithShadow(matrices, "Hunger Display - Colours", (float)(this.width/2 - this.textRenderer.getWidth("Hunger Display - Colours")/2), 16.0f, 0xFFAA00);
            this.textRenderer.drawWithShadow(matrices, "Saturation pip colour (hex RRGGBB)", (float)(cx()-130), 48.0f, 0xCCCCCC);
            this.textRenderer.drawWithShadow(matrices, "Exhaustion bar colour (hex RRGGBB)", (float)(cx()-130), 86.0f, 0xCCCCCC);
            this.textRenderer.drawWithShadow(matrices, "Food preview pip colour (hex RRGGBB)", (float)(cx()-130), 124.0f, 0xCCCCCC);
            super.render(matrices, mx, my, delta);
        ...
        @Override
        public void onClose() { MinecraftClient.getInstance().openScreen(parent); }
    }
}
