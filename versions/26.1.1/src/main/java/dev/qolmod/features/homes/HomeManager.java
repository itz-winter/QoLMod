...
public class HomeManager {
    ...
    public HomeManager(MinecraftServer server) {
        this.homesDir = server.getSavePath(LevelResource.ROOT).resolve("qolmod_homes");
        loadAll();
    }

    public void setHome(UUID playerId, String name, ResourceKey<World> dimension, BlockPos pos, float yaw, float pitch) {
    ...
    private void loadPlayerFile(Path file) {
        try (Reader reader = Files.newBufferedReader(file, StandardCharsets.UTF_8)) {
            ...
            for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                ...
                ResourceKey<World> dimension = ResourceKey.create(ResourceKeys.WORLD, ResourceLocation.of(dimId));
    ...
    public static class HomeData {
        ...
        public final ResourceKey<World> dimension;
        public final BlockPos pos;
        public final float yaw;
        public final float pitch;

        public HomeData(String name, ResourceKey<World> dimension, BlockPos pos, float yaw, float pitch) {
            this.name = name;
