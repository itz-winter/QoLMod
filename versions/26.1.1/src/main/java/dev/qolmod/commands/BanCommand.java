...
public class BanCommand {
    ...
    private static int executeBan(CommandContext<ServerCommandSource> ctx, String durationStr, String reason) {
        ...
        ServerPlayer target = server.getPlayerManager().getPlayer(targetName);
        if (target == null) {
            source.sendMessage(Component.literal("§cPlayer not found: " + targetName));
            return 0;
        }
        ...
        if (durationStr != null && !durationStr.equalsIgnoreCase("permanent")) {
            ...
            if (seconds <= 0) {
                source.sendMessage(Component.literal("§cInvalid duration format. Use: 1d2h30m"));
                return 0;
            }
        ...
        target.networkHandler.disconnect(Component.literal("§cYou have been banned " + durationDisplay +
                ".\n§7Reason: " + reasonStr));

        source.sendMessage(Component.literal("§aBanned " + targetName + " " + durationDisplay + "."));
        if (reason != null) {
            source.sendMessage(Component.literal("§7Reason: " + reasonStr));
        }
    ...
    private static int executeUnban(CommandContext<ServerCommandSource> ctx) {
        ...
        if (profile.isPresent()) {
            if (banList.contains(profile.get())) {
                ...
                source.sendMessage(Component.literal("§aUnbanned " + targetName + "."));
                return 1;
            }
        }

        source.sendMessage(Component.literal("§c" + targetName + " is not banned."));
        return 0;
    }
