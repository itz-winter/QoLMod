...
public class TreeChopperManager {
     ...
     */
    public boolean onBlockBreak(World world, Player player, BlockPos pos, BlockState state) {
        ...
        if (config.treeChopperFastLeafDecay) {
            scheduleLeafDecay((ServerLevel) world, validation.logs, bottomPos);
        }
                ...
                for (int z = -2; z <= 2; z++) {
                    ...
                    if (isLeaf(block, config)) {
                        ...
                        if (config.treeChopperIgnorePlayerMade) {
                            boolean persistent = checkState.hasProperty(BlockStateProperties.PERSISTENT)
                                    && checkState.getValue(BlockStateProperties.PERSISTENT);
                            if (persistent) {
    ...
    private void harvestTree(World world, ServerPlayer player, List<BlockPos> logs, QoLConfig config) {
        ...
        for (BlockPos logPos : logs) {
            ...
            Block.dropResources(state, world, logPos, world.getBlockEntity(logPos), player, axe);
            ...
            if (!player.isCreative() && axe.getItem() instanceof AxeItem) {
                counter++;
                if (counter >= lossPer) {
                    axe.damage(1, ServerLevel, player, (item) -> {});
                    counter = 0;
                    if (axe.isEmpty()) break; // Axe broke
                }
            }
    ...
    private void scheduleLeafDecay(ServerLevel world, List<BlockPos> logs, BlockPos bottomPos) {
        ...
        for (BlockPos pos : BlockPos.betweenClosed(
            ...
            if (isLeaf(state.getBlock(), QoLMod.getConfig())) {
                world.scheduleTick(pos, state.getBlock(), world.getRandom().nextInt(20) + 1);
            }
        }
    }
    ...
    public static boolean isLog(Block block) {
        ...
        return block instanceof RotatedPillarBlock && (name.contains("log") || name.contains("stem") || name.contains("wood"))
    ...
    private boolean areSameWoodType(Block a, Block b) {
        String nameA = a.getDescriptionId().split("\\.")[2]; // e.g. "oak_log" -> first word
        String nameB = b.getDescriptionId().split("\\.")[2];
    ...
    private boolean isLeaf(Block block, QoLConfig config) {
        ...
        if (config.treeChopperHugeMushrooms && block instanceof HugeMushroomBlock) return true;
        return false;
    }
