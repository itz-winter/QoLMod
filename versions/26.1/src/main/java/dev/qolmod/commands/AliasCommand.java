...
public class AliasCommand {
    ...
    private static int executeAdd(CommandContext<ServerCommandSource> ctx) {
        ...
        player.sendMessage(Component.literal("§aAlias added: §f/" + name + " §7→ §f/" + command));
        return 1;
    }
    ...
    private static int executeRemove(CommandContext<ServerCommandSource> ctx) {
        ...
        if (hadAlias) {
            player.sendMessage(Component.literal("§aAlias removed: §f/" + name));
            return 1;
        } else {
            player.sendMessage(Component.literal("§cAlias not found: " + name));
            return 0;
        }
    }
    ...
    private static int executeList(CommandContext<ServerCommandSource> ctx) {
        ...
        if (aliases.isEmpty()) {
            player.sendMessage(Component.literal("§7You have no aliases. Use §e/alias add <name> <command>§7."));
            return 0;
        }

        player.sendMessage(Component.literal("§6=== Your Aliases ==="));
        for (Map.Entry<String, String> entry : aliases.entrySet()) {
            player.sendMessage(Component.literal("§e/" + entry.getKey() + " §7→ §f/" + entry.getValue()));
        }
        return 1;
    }
